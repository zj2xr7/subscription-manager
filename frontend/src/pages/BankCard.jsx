...
export default function BankCard({ balance, lots, transactions, subscriptions, onQuote, onDeposit, onDeleteDeposit }) {
  ...
  const removeDeposit = item => {
    const impact = item.related_charge_count > 0
      ? `该批次已使用 ${item.used_usdt.toFixed(4)} USDT，删除会同步清理 ${item.related_charge_count} 条关联扣款并回退相关订阅账期。`
      : '该批次尚未参与扣款，删除后余额会同步减少。'
