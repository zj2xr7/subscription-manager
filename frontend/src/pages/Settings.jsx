...
export default function Settings({ settings, exchangeQuotes, onSaveNotification, onSaveExchange, onTest, onRefreshRates }) {
  ...
  const notificationDirty = notificationForm.server_chan_key.trim() !== (settings.server_chan_key || '')
    || notificationForm.notification_days_before !== (settings.notification_days_before ?? 7)
  ...
  return <main className="settings-page"><div className="page-head"><div><p className="eyebrow">PREFERENCES</p><h1>设置</h1><p>管理续费通知与货币换算数据。</p></div></div>
    <section className="panel settings-section"><div className="setting-section-head"><div className="setting-icon purple">♢</div><div><h2>Server 酱通知</h2><p>订阅到期前，通过微信接收续费提醒。</p></div></div><div className="setting-fields"><label><span>SendKey</span><small>用于向你的 Server 酱账号发送消息</small><input type="password" name="server_chan_key" value={notificationForm.server_chan_key} onChange={event => setNotificationForm(value => ({ ...value, server_chan_key: event.target.value }))} placeholder="SCT..." /></label><label><span>提前通知天数</span><small>到期前多少天发送一次提醒</small><input type="number" name="notification_days_before" value={notificationForm.notification_days_before} onChange={event => setNotificationForm(value => ({ ...value, notification_days_before: Number(event.target.value) }))} min="0" max="90" /></label></div><div className="setting-actions split-actions"><button type="button" className="secondary compact-button" disabled={testing || savingNotification || notificationDirty || !settings.server_chan_key} onClick={async () => { setTesting(true); try { await onTest() } finally { setTesting(false) } }}>{testing ? '发送中…' : '发送测试通知'}</button><button type="button" className="primary compact-button" disabled={savingNotification || !notificationDirty} onClick={saveNotification}>{savingNotification ? '保存中…' : '保存通知设置'}</button></div></section>
