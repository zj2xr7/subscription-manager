...
export default function DepositForm({ balance, subscriptions, onQuote, onSubmit }) {
  ...
  return <form className="panel deposit-form" onSubmit={async event => { event.preventDefault(); setSaving(true); try { await onSubmit({ cny_amount: Number(cnyAmount), c2c_rate: Number(rate) }); setCnyAmount(''); setSelected([]) } finally { setSaving(false) } }}>
    ...
    {selected.length > 0 && quote && <div className="topup-suggestion"><div><span>所选服务需要</span><b>{quote.required_usdt.toFixed(4)} USDT</b></div><div><span>当前可用余额</span><b>{balance.toFixed(4)} USDT</b></div><div><span>余额缺口</span><b>{quote.shortfall_usdt.toFixed(4)} USDT</b></div><div className="suggestion-total"><span>建议充值</span><strong>¥{quote.suggested_cny_amount.toFixed(2)}</strong></div><button type="button" className="text-fill" disabled={!quote.suggested_cny_amount} onClick={() => setCnyAmount(quote.suggested_cny_amount.toFixed(2))}>填入建议金额</button></div>}
