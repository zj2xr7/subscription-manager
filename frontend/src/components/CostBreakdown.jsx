export default function CostBreakdown({ cost }) {
  ...
  return <div className="cost-breakdown">
    ...
    {bank && <>
      ...
      {cost.shortfall_usdt > 0 && <><div className="cost-shortfall"><span>余额缺口</span><b>{cost.shortfall_usdt.toFixed(4)} USDT</b></div><div><span>缺口参考估值</span><b>{cost.shortfall_usdt.toFixed(4)} × ¥{cost.estimate_cny_rate.toFixed(4)}</b></div><div><span>估值来源</span><b>{cost.estimate_source === 'api' ? '实时 USD/CNY' : '参考 USD/CNY'}</b></div></>}
    </>}
