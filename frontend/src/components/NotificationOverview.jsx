const statusText = {
  never: '尚未执行', running: '检查中', success: '运行正常', failed: '存在失败', disabled: '通道未启用',
}
...
export default function NotificationOverview({ data }) {
  ...
  return <section className="panel notification-panel">
    <div className="section-title"><div><p className="eyebrow">NOTIFICATIONS</p><h2>通知状态</h2></div><span className={`notification-state ${data?.enabled && scheduler?.running ? 'ok' : 'off'}`}>{data?.enabled ? (scheduler?.running ? '运行中' : '调度未运行') : '未配置 SendKey'}</span></div>
    <div className="notification-metrics">
      ...
      <div><span>本次结果</span><b>{scheduler ? `${scheduler.sent_count} 成功 · ${scheduler.failed_count} 失败 · ${scheduler.due_count} 应发送` : '—'}</b></div>
    </div>
    {scheduler?.error_message && <div className="notification-error">{scheduler.error_message}</div>}
